...
exports.resolveIssue = async(req, res) => {
    ...
    try {
        const admin = await Admin.findOne({ adminId: adminID });
        const userWithIssue = await User.findOne({ 'issues.issue.issueID': issueID });
        if (!userWithIssue) {
            return res.status(400).json({ error: 'Issue not found for any user' });
        }
        const issue = userWithIssue.issues.find(issue => issue.issue.issueID === issueID);
        if (!issue) {
            return res.status(400).json({ error: 'Issue not found for the user' });
        }
        if (!issue.pertaining) {
            return res.status(200).json({ message: 'Issue already resolved', userBadgeID: userWithIssue.badgeID });
        }
        await User.updateOne({ 'issues.issue.issueID': issueID }, { $set: { 'issues.$.pertaining': false } });
        return res.status(200).json({ message: 'Issue resolved successfully', userBadgeID: userWithIssue.badgeID });
    } catch (error) {
        console.error(error);
        return res.status(500).json({ error: 'An error occurred while resolving the issue' });
    }
};
...
exports.getCurrentOfficers = async(req, res) => {
    ...
    try {
        ...
        const officersBadgeIDs = new Set();
        for (const session of sessions) {
            const users = await User.find({ 'sessions.session': session._id }, 'badgeID');
            for (const user of users) {
                officersBadgeIDs.add(user.badgeID);
            }
        }

        res.json({
            officersBadgeIDs: [...officersBadgeIDs],
        });
    } catch (error) {
